...
//     var dates = DateFormat('dd/MM/yyyy');
//     currentDate = dates.format(cDate);
//     print("checking dates here ${currentDate}");
//   }
//
...
//     _isNetworkAvail = await isNetworkAvailable();
//     print('table id here now ${TABLE_ID}');
//     if (_isNetworkAvail) {
//       try {
...
//             _controller.add(new TextEditingController());
//             sellerId = cartList[i].productList![0].seller_id;
//             print("ok seller id here ${sellerId}");
//           }
//         } else {
...
//                 cartList[index].qty = qty.toString();
//               }
//               print("double price here ${data[SUB_TOTAL]}");
//               oriPrice = data[SUB_TOTAL] == null || data[SUB_TOTAL] == "" ? 0.0 : double.parse(data[SUB_TOTAL]);
//               print("ddss ${oriPrice}");
//               if (!ISFLAT_DEL) {
//                 try {
//                   if ((oriPrice) <
...
//
//             totalPrice = double.parse(data["final_total"]) + 0.0;
//
...
//List<SectionModel> cartList = [];
List<Promo> promoList = [];
double totalPrice = 0, oriPrice = 0, delCharge = 0, taxPer = 0,service_charge = 0 ;
int? selectedAddress = 0;
String?  payMethod = '', selTime, selDate, promocode;
String selAddress = "d";
bool? isTimeSlot,
    isPromoValid = false,
    isUseWallet = false,
...
class StateCart extends State<Cart> with TickerProviderStateMixin {
  ...
  //List<PaymentItem> _gpaytItems = [];
  //Pay _gpayClient;
bool isTakeway = false;
  getSharedData()async{
  SharedPreferences prefs = await SharedPreferences.getInstance();
  isTakeway =  prefs.getBool('isTakeAway')!;
  String? ok = prefs.getString('sellerId');
  print("checking isTakeWay ${isTakeway} and ${ok}");
  }
  ...
  callWaiter()async{
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String? sid = prefs.getString('sellerId');
    String? oId = prefs.getString('tableId');
    print("checking both ids here ${result['sellerId']} and ${result['tableId']}");
    var headers = {
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                Text(
                  "${cartList[index].productList![0].service_charge!}" + "%",
                  style: TextStyle(
                      color: Theme.of(context).colorScheme.lightBlack2),
                )
    ...
    return _isCartLoad
      ...
      children: <Widget>[
        ...
        Container(
          ...
          //         ),
          //         child:Text("Call a waiter",style: TextStyle(color: Colors.white,fontSize: 16,fontWeight: FontWeight.w500),)),
          //   ),
                       SimBtn(
                size: 0.9,
                title: getTranslated(context, 'PROCEED_CHECKOUT'),
                ...
                result =   await Navigator.push(context, MaterialPageRoute(builder: (context) =>
                   QRViewExample(
                   isTakeAway: isTakeway,
                   )));
                print("checking result here ${result['sellerId']} and ${result}");
                if(result['sellerId'] == null || result['sellerId'] == ""){
                  ...
                  //
                  //                 });
                  //                 if(result['tableId'] == null || result['tableId'] == ""){
                  //                   callWaiter(result['sellerID'],"");
                  //                   print("eee");
                  //                   if (oriPrice > 0) {
                  //
                  //                     FocusScope.of(context).unfocus();
                  ...
                  //                   print("ooo");
                  //
                  //                   callWaiter(result['sellerID'],result['tableId']);
                  //                   if (oriPrice > 0) {
                  //                     FocusScope.of(context).unfocus();
                  //                     if (isAvailable) {
                  //                         print("jjj");
                                  ...
                                  child: Padding(
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                       ...
                                       // / promo(),
                                        orderSummary(cartList),
                                        isTakeway == true ? SizedBox.shrink() :  InkWell(
                                          onTap:(){
                                            waiterFunction();
                                          },
                                          child: Container(
                          ...
                          Container(
                            ...
                            isTakeway == true ?  SimBtn(
                                  ...
                                  _placeOrder
                                    ...
                                    //   _placeOrder = false;
                                    // });
                                    // if(isWaiter == null || isWaiter == ""){
                                    //  setState((){
                                    //    callWaiter();
                                    //  });
                                    //  print("working here call waiter");
                                    // }
                                     if (selAddress == null ||
                                        selAddress.isEmpty) {
                                      msg = getTranslated(
                            ...
                            SimBtn(
                                ...
                                onBtnSelected: ()async{
                                  SharedPreferences prefs = await SharedPreferences.getInstance();
                                  String? sId = prefs.getString("seller_id");
  ...
  Future<void> placeOrder(String? tranId) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    // String sId = prefs.getString('seller_id')!;
    // bool isTakeAway = prefs.getBool('isTakeAway')!;
    // print("PLACE OREDR ${sId}");
    _isNetworkAvail = await isNetworkAvailable();
    if (_isNetworkAvail) {
      context.read<CartProvider>().setProgress(true);
      ...
      try {
        var parameter = {
          ...
          "order_type": isTakeway == true ? "1:":"2",
          'table_no':'${TABLE_ID}',
          'order_date':'${currentDate}',
          'seller_id': isTakeway == true ? "${sellerId}" : "${sellerId}",
          PAYMENT_METHOD: isTakeway == true ? payVia : 'COD',
       //   ADD_ID: selAddress,
          ADD_ID: "0",
          ISWALLETBALUSED: isUseWallet! ? "1" : "0",
          WALLET_BAL_USED: usedBal.toString(),
