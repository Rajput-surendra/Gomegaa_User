...
import 'package:eshop_multivendor/Model/UpdateCompleteStatusModel.dart';
...
class StateOrder extends State<OrderDetail>
  ...
  _launchCaller(mobile) async {
    ...
    updateStatusFunction()async{
    print("order id here ${widget.model!.id}");
      var headers = {
        ...
        'Cookie': 'ci_session=ea208113296d6f0de964de00afcaa4d670e92aff'
      };
      var request = http.MultipartRequest('POST', Uri.parse(baseUrl+'update_complete_status'));
      ...
      if (response.statusCode == 200) {
        ...
        final jsonResponse = UpdateCompleteStatusModel.fromJson(json.decode(result));
        print("ok here ${jsonResponse.error}");
        setState(() {
          orderStatus = jsonResponse.error!;
        });
...
/*  returnable() {
  ...
  productItem(OrderItem orderItem, OrderModel model) {
    ...
    int one = 00;
    int two = 00;
  if(orderItem.estitmateTime == "" || orderItem.estitmateTime!.length == 1){
     minut = one;
     second = two;
  }
  else{
    List splitdata = orderItem.estitmateTime!.split(":");
     minut = int.parse(splitdata[0].toString());
                            ...
                            Row(
                              children: [
                                Column(
                                  ...
                                  children: [
                                    Text("Estimate Time"),
                                   orderItem.estitmateTime == "" ? Text("00:00 mins") : Text("${orderItem.estitmateTime} mins")
                                  ],
                                ),
                                // Text("${_current}")
                              ],
                            ),
                          orderItem.estitmateTime == "" ? SizedBox.shrink() :  Text("Your order will be ready in.."),
                            orderItem.estitmateTime == "" ? SizedBox.shrink() : TimerCountdown(
...
*/
  ...
  DwnInvoice() {
    print("Invoicccccccccccccccccc${widget.model!.invoice!}");
    return Card(
      elevation: 0,
      child: InkWell(
          child: ListTile(
  ...
  getOrderDetails(OrderModel model) {
    return SingleChildScrollView(
      ...
      child: Padding(
        ...
        widget.model!.userCompleteStatus == "1" ?   DwnInvoice() : SizedBox.shrink(),
            //deliveryBoyDetails(),
          //  shippingDetails(),
            priceDetails(),
            widget.model!.userCompleteStatus == "1" ?
                SizedBox.shrink()
            : InkWell(
              onTap: (){
              ...
              child: Container(
                ...
                child: Text("Order Done",style: TextStyle(color: Colors.white,fontWeight: FontWeight.w500,fontSize: 16),),
              ),
            ),
          ],
        ),
